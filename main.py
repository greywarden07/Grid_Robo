...
expiry_pattern = r"(?:\b(?:Exp(?:iry)?[: ]?)?(?:\d{2}[\/\-]\d{2}[\/\-]\d{4})|" \
                 r"(?:\d{2}[\/\-]\d{4})|" \
                 r"(?:\d{4}[\/\-]\d{2})|" \
                 r"(?:\b(?:Exp(?:iry)?[: ]?)?(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[ ]?\d{4})\b)"
...
def extract_product_details(text):
    ...
    # Regex for MRP, expiry date, and pack size
    mrp_pattern = r"(₹|Rs\.?)[ ]?\d+(\.\d{1,2})?"
    pack_size_pattern = r"(\d+\s?(g|kg|ml|L|litre|liters|gm|grams))"
    ...
    for line in lines:
        if re.match(r"^[a-zA-Z\s]+$", line):  # Only letters and spaces allowed for brand name
